	...
	*/
	...
	var unit = function(opts) {
		...
		this._addUnit = function(n) {
			...
			if (round >= 0 && _k) { //有单位时才保留小数
				_new = this.decimal(_new, round);
			};
		...
		//保留几位小数
		this.decimal = function(n, c) {
			if (!c || c < 0) c = 0;
			var n = Math.round(n * Math.pow(10, c)) / Math.pow(10, c);
			//t = n.toFixed(c);
			return n;
		};
		...
		//添加后缀
		/* 不换算单位 例: 123456789TH - >123,456,789TH
		if(_key.n != null){
